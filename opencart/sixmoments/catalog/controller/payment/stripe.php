...
class Stripe extends \Opencart\System\Engine\Controller {
    ...
    private function createSession(array $order): array {
        $key=(string)$this->config->get('payment_sixmoments_stripe_secret_key');$success=$this->url->link('extension/sixmoments/payment/stripe.success','language='.$this->config->get('config_language').'&session_id={CHECKOUT_SESSION_ID}',true);$cancel=$this->url->link('checkout/checkout','language='.$this->config->get('config_language'),true);
