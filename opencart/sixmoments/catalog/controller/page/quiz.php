...
class Quiz extends \Opencart\System\Engine\Controller {
    public function index(): void {
        ...
        $data['products']=[]; $this->load->model('catalog/product');$this->load->model('extension/sixmoments/catalog');
        $starter_tags=['RI'=>'ring,yellow-gold','WE'=>'ring,rose-gold','NE'=>'necklace,white-gold','EA'=>'earring,yellow-gold','BR'=>'bracelet,rose-gold'];
        foreach($this->model_extension_sixmoments_catalog->getProductIds(['sort'=>'popular','start'=>0,'limit'=>48]) as $product_id){$product=$this->model_catalog_product->getProduct($product_id);if(!$product)continue;$parts=explode('-',(string)$product['model']);$tags=trim((string)$product['tag'].','.(string)($starter_tags[$parts[1]??'']??''),',');$data['products'][]=['card'=>$this->productThumb($product,$tags)];}
        $rules=json_decode((string)$this->config->get('module_sixmoments_quiz_rules'),true);$data['quiz_rules']=json_encode(is_array($rules)?$rules:[],JSON_UNESCAPED_UNICODE|JSON_HEX_APOS|JSON_HEX_QUOT);
        $data['newsletter_action']=$this->url->link('extension/sixmoments/newsletter.subscribe','language='.$this->config->get('config_language'));
