...
class Catalog extends \Opencart\System\Engine\Controller {
    public function index(): void {
        ...
        $this->document->setTitle($this->language->get('six_catalog_title') . ' | 6MOMENTS');
        ...
        $total = $this->model_extension_sixmoments_catalog->getTotalProducts($filter);
        $data['total'] = $total;
        $data['filters'] = $filter;
    ...
    private function filterUrl(array $replace): string {
        ...
        return $this->url->link('extension/sixmoments/page/catalog', 'language=' . $this->config->get('config_language') . $suffix);
    }
