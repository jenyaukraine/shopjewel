    ...
    public function suggest(): void {
        ...
        if (oc_strlen($q) >= 2) {
            ...
            foreach ($this->model_extension_noveraile_catalog->getProductIds($filter) as $product_id) {
                ...
                $market_price = $this->model_extension_noveraile_pricing->resolve($product, $currency);
                $price = (float)($market_price['special'] ?: $market_price['price']);
                $json['results'][] = [
                    'name' => $product['name'], 'model' => $product['model'],
                    'image' => $this->model_tool_image->resize($image, 120, 120),
                    'price' => $this->model_extension_noveraile_pricing->format($price, $currency, $market_price['fixed']),
